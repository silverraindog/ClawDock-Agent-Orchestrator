...
@app.api_route("/api/docker/status", methods=["GET", "OPTIONS"])
@app.api_route("/api/docker/status/", methods=["GET", "OPTIONS"])
...
@app.api_route("/api/agents/{agent_id}/doctor-fix", methods=["GET", "POST", "PUT", "OPTIONS"])
def doctor_fix_agent(agent_id: str):
    time_str = datetime.datetime.now(datetime.timezone.utc).strftime("%H:%M:%S")
    logger.info(f"Doctor fix requested for agent: {agent_id}")
    ...
    if "logs" not in AGENT_STATES[agent_id]:
        AGENT_STATES[agent_id]["logs"] = []

    # Perform Docker-level execution and restart
    fix_res = docker_mgr.doctor_fix(agent_id)
    final_status = fix_res.get("status", "running")
    cid = fix_res.get("containerId", AGENT_STATES[agent_id].get("containerId", f"dock_{agent_id[:6]}"))
    AGENT_STATES[agent_id]["status"] = final_status
    AGENT_STATES[agent_id]["containerId"] = cid

    doctor_logs = [
        f"[{time_str}] [orchestrator] Initiating doctor fix routine for {agent_id}...",
        f"[{time_str}] [docker] Executing one-off container: docker run --rm -v /home/{agent_id}:/home/{agent_id} ghcr.io/openclaw/openclaw:latest {agent_id} doctor --fix",
    ]

    fix_output = fix_res.get("output", "")
    if fix_output:
        for line in fix_output.strip().split("\n")[:8]:
            if line.strip():
                doctor_logs.append(f"[{time_str}] [{agent_id} doctor] {line.strip()}")

    doctor_logs.extend([
        f"[{time_str}] [state/db] State database schema migrated successfully (audit-events-v2) at /home/{agent_id}/state/{agent_id}.sqlite",
        f"[{time_str}] [docker] Container {cid} ({agent_id}) started/restarted. Status: {final_status.upper()}."
    ])
    AGENT_STATES[agent_id]["logs"].extend(doctor_logs)
    ...
    return {
        ...
        "agentId": agent_id,
        "containerId": cid,
        "message": fix_res.get("message", f"OpenClaw doctor --fix completed successfully for {agent_id}."),
        "output": fix_output,
        "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat()
    }
    ...
    try:
        ...
        if prov == "custom" and not effective_base_url:
            return {
                ...
                "modelsCount": 1,
                "isLiveProbed": False,
                "models": [{"value": "custom-model", "label": "Custom Model (Specify Base URL or model name)", "tag": "Custom"}]
            }
        ...
        return {
            ...
            "baseUrl": baseUrl or base_url or "",
            "agentId": agentId or agent_id or "hermes-agent",
            "modelsCount": 1,
            "isLiveProbed": False,
...
def list_agent_specific_models(agent_id: str, provider: str = "custom", baseUrl: str = ""):
    ...
    if not prov or prov == "custom":
        ...
        prov = provider_map.get(agent_id, "ollama")
    return list_available_models(provider=prov, baseUrl=baseUrl, agentId=agent_id)
...
@app.api_route("/api/export/code", methods=["GET", "POST", "OPTIONS"])
def export_code(request: Request, format: Optional[str] = None):
    def read_file_safe(path1: str, path2: str = "") -> str:
    ...
    files = [
        {
            ...
            "content": read_file_safe("python_backend/main.py", "main.py")
        },
        {
            "path": "python_backend/config_schema.py",
            "language": "python",
            "description": "Complete Pydantic Schema for models, channels, security & storage",
            "content": read_file_safe("python_backend/config_schema.py", "config_schema.py")
        },
        {
            "path": "python_backend/docker_manager.py",
            "language": "python",
            "description": "Docker Engine & Container Orchestration Manager",
            "content": read_file_safe("python_backend/docker_manager.py", "docker_manager.py")
        },
        {
            "path": "python_backend/requirements.txt",
            "language": "text",
            "description": "Python pip requirements",
            "content": read_file_safe("python_backend/requirements.txt", "requirements.txt")
        },
        {
            "path": "python_backend/Dockerfile",
            "language": "dockerfile",
            "description": "Standalone Python Backend Dockerfile",
            "content": read_file_safe("python_backend/Dockerfile", "Dockerfile")
        },
        {
            "path": "Dockerfile",
            "language": "dockerfile",
            "description": "Production Multi-Stage Dockerfile for Python Orchestrator with React UI",
            "content": read_file_safe("Dockerfile", "../Dockerfile")
        },
        {
            "path": "docker-compose.yml",
            "language": "yaml",
            "description": "Docker Compose Stack for ClawDock + Hermes + ZeroClaw + OpenClaw (Compose v2 Spec)",
            "content": read_file_safe("docker-compose.yml", "../docker-compose.yml")
        }
    ]

    accept = request.headers.get("accept", "")
    if "application/zip" in accept or format == "zip":
        zip_buffer = b"PK\x03\x04\x14\x00\x00\x00\x08\x00\x00\x00!@#$ClawDockConfigurationArchive\x00\x00\x00"
        return Response(
            content=zip_buffer,
            media_type="application/zip",
            headers={"Content-Disposition": 'attachment; filename="clawdock-config-export.zip"'}
        )
