...
export const DockerLogsInspector: React.FC<DockerLogsInspectorProps> = ({
  ...
  const handleRunDoctorFix = async () => {
    ...
    try {
      ...
      // If server returns 405 Method Not Allowed or 404, fallback gracefully
      ...
      if (res.ok && (data?.success || data?.status === 'running')) {
        if (onAddToast) onAddToast('success', 'OpenClaw Doctor Fixed', data?.message || 'Database migrated to audit-events-v2 successfully.');
        fetchLogs(true);
        if (onRestartContainer) onRestartContainer();
      } else {
        if (onAddToast) onAddToast('error', 'Doctor Fix Failed', data?.error || data?.message || 'Failed to run openclaw doctor --fix');
      }
    } catch (e: any) {
      if (onAddToast) onAddToast('error', 'Doctor Fix Failed', e.message);
    } finally {
      setIsFixingDoctor(false);
    }
  };

  const hasMigrationError = logs.some(l => 
    l.includes('OpenClawStateDatabaseSchemaMigrationRequiredError') || 
    l.includes('audit-events-v2') || 
    l.includes('openclaw doctor --fix')
  );
  ...
  return (
    <div className={`rounded-2xl border border-slate-800 bg-slate-900/90 shadow-xl overflow-hidden transition-all ${
      ...
      {/* Migration Error Action Banner for OpenClaw */}
      {hasMigrationError && (
        <div className="p-3.5 bg-amber-500/10 border-b border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-3 px-4">
          <div className="flex items-center gap-2.5 text-amber-300 text-xs font-medium">
            <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
            <span>OpenClaw database schema migration required (audit-events-v2) at <code className="font-mono text-amber-200">/home/openclaw/state/openclaw.sqlite</code>.</span>
          </div>
          <button
            id="openclaw-doctor-fix-btn"
            onClick={handleRunDoctorFix}
            disabled={isFixingDoctor}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow-sm transition-colors disabled:opacity-50 shrink-0 cursor-pointer"
          >
            <RotateCw className={`w-3.5 h-3.5 ${isFixingDoctor ? 'animate-spin' : ''}`} />
            <span>{isFixingDoctor ? 'Running openclaw doctor --fix...' : 'Run openclaw doctor --fix'}</span>
          </button>
        </div>
      )}
