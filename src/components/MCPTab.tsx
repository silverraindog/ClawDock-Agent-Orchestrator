...
export const MCPTab: React.FC<MCPTabProps> = ({
  ...
  const filteredServers = (mcpServers || []).filter(s => {
    ...
    const toolsMatch = Array.isArray(s.toolsProvided) && s.toolsProvided.some(t => (t || '').toLowerCase().includes(query));
    return nameMatch || descMatch || toolsMatch;
  });

  const activeCount = (mcpServers || []).filter(s => Boolean(s?.enabled)).length;
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 sm:p-6 space-y-5">
        ...
        <div className="p-3.5 rounded-xl bg-gradient-to-r from-cyan-950/40 via-indigo-950/30 to-slate-900 border border-cyan-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          >
            ...
            <span>{isSyncingRemote ? 'Fetching MCP Servers...' : 'Sync MCP Servers from OpenClaw VPS'}</span>
          </button>
        </div>
                ...
                <div className="p-2 rounded-lg bg-slate-900 border border-slate-800/80 font-mono text-[11px] text-indigo-300 truncate">
                  {server.url ? (
                    ...
                    <div className="truncate">
                      ...
                      <span>{server.command || 'mcp-server'}</span>
