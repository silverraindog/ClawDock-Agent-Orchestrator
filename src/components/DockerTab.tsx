...
export const DockerTab: React.FC<DockerTabProps> = ({
  ...
  const handleDoctorFix = async (agentId: AgentId) => {
    setIsDoctorFixing(agentId);
    try {
      let res = await fetch(`/api/agents/${agentId}/doctor-fix`, { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'doctor-fix' })
      });

      // If older container or proxy returns 405 Method Not Allowed or 404, fallback gracefully
      if (res.status === 405 || res.status === 404) {
        try {
          const getRes = await fetch(`/api/agents/${agentId}/doctor-fix`);
          if (getRes.ok) {
            res = getRes;
          } else {
            res = await fetch(`/api/agents/${agentId}/restart`, { method: 'POST' });
          }
        } catch {
          res = await fetch(`/api/agents/${agentId}/restart`, { method: 'POST' });
        }
      ...
      if (res.ok && (data?.success || data?.status === 'running')) {
        onAddToast('success', 'OpenClaw Doctor Fixed', data?.message || 'Database schema migration completed successfully (audit-events-v2) and container restarted.');
        if (onRestartAgent) onRestartAgent(agentId);
      } else {
        onAddToast('error', 'Doctor Fix Failed', data?.error || data?.message || 'Could not run doctor fix');
      }
    } catch (e: any) {
      onAddToast('error', 'Doctor Fix Failed', e.message);
    } finally {
      setIsDoctorFixing(null);
    }
  };
...
${agents.map(a => `  ${a.id}:
    image: ${a.dockerImage}
    container_name: ${a.containerName || `${a.id}-runtime`}
    ports:
      - "${a.defaultPort}:${a.defaultPort}"
    volumes:
      - ./workspace:/workspace
    restart: unless-stopped
    networks:
...
networks:
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* OpenClaw Schema Migration Doctor Fix Banner */}
      <div className="p-5 rounded-2xl border border-amber-500/40 bg-gradient-to-r from-amber-950/40 to-slate-900 border-l-4 border-l-amber-500 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-lg">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-sm font-bold text-amber-300">
            <AlertTriangle className="w-4 h-4 text-amber-400" />
            OpenClaw Database Schema Migration Required (audit-events-v2)
          </div>
          <p className="text-xs text-slate-300 max-w-xl">
            OpenClaw gateway exited with code 78 due to pending state database migration at <code className="font-mono text-amber-200">/home/openclaw/state/openclaw.sqlite</code>. Click <strong>Doctor Fix</strong> to run <code className="font-mono text-amber-200">openclaw doctor --fix</code>.
          </p>
        </div>
        <button
          id="top-openclaw-doctor-fix-btn"
          onClick={() => handleDoctorFix('openclaw')}
          disabled={isDoctorFixing === 'openclaw'}
          className="shrink-0 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow-md transition-all cursor-pointer disabled:opacity-50"
        >
          <RotateCw className={`w-4 h-4 ${isDoctorFixing === 'openclaw' ? 'animate-spin' : ''}`} />
          <span>{isDoctorFixing === 'openclaw' ? 'Running openclaw doctor --fix...' : 'Run openclaw doctor --fix'}</span>
        </button>
      </div>
              ...
              >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="text-[11px] text-indigo-300 font-mono mt-1 truncate flex items-center gap-1.5">
                        <Container className="w-3 h-3 text-indigo-400 shrink-0" />
                        <span className="truncate">{agent.dockerImage}</span>
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between gap-2 pt-4 mt-3 border-t border-slate-800">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {agent.id === 'openclaw' && (
                      <button
                        id="openclaw-card-doctor-fix-btn"
                        onClick={() => handleDoctorFix(agent.id)}
                        disabled={isDoctorFixing === agent.id}
                        className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold text-slate-950 bg-amber-400 hover:bg-amber-300 transition-colors shadow-sm cursor-pointer disabled:opacity-50"
                        title="Run openclaw doctor --fix to migrate state database schema (audit-events-v2)"
                      >
                        <RotateCw className={`w-3.5 h-3.5 ${isDoctorFixing === agent.id ? 'animate-spin' : ''}`} />
                        <span>{isDoctorFixing === agent.id ? 'Fixing...' : 'Doctor Fix'}</span>
                      </button>
                    )}
                  </div>
                </div>
