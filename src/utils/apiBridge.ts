  ...
  for (const endpoint of candidateEndpoints) {
    ...
    if (result.ok && result.data && result.data.success) {
      return {
        ...
        mcpServers: Array.isArray(result.data.mcpServers)
          ? result.data.mcpServers.map((m: any) => ({
              ...m,
              command: m.command || (m.url ? 'curl' : 'mcp-server'),
...
/**
 * Direct browser probe has been deprecated in favor of server-side proxy probing (/api/models)
 * to avoid browser cross-origin CORS policy blocks, 403 Forbidden, and connection refused console noise.
 */
export async function probeBrowserDirectLiveModels(_baseUrl: string, _provider: string): Promise<ModelOptionItem[] | null> {
  // All probing is safely routed via server-side /api/models to eliminate browser CORS errors
  return null;
}

/**
 * Dedicated backend proxy fetcher for models from a baseUrl to bypass browser CORS / 403 Forbidden restrictions.
 */
  ...
  try {
    ...
    if (res.ok) {
      ...
      return {
        ...
        models: data.models || [],
        rawModelNames: data.rawModelNames || [],
        source: data.source || 'backend_proxy'
      };
    }
  } catch (err) {
    console.warn('[ProxyModels] Failed to fetch through backend proxy:', err);
  }
  return {
    success: false,
    models: [],
    rawModelNames: [],
    source: 'backend_proxy_error'
  };
}
...
export function saveLocalAgentStates(states: Record<string, any>): void {
  try {
    if (states && typeof states === 'object') {
      const existing = getLocalAgentStates() || {};
      const merged = { ...existing, ...states };
      localStorage.setItem(AGENT_STATES_LOCAL_KEY, JSON.stringify(merged));
    }
  } catch {}
}

export function getLocalUpdates(): SystemUpdateItem[] | null {
  try {
    const raw = localStorage.getItem(SYSTEM_UPDATES_LOCAL_KEY);
    if (raw) {
      const parsed = JSON.parse(raw);
...
export function saveLocalUpdates(updates: SystemUpdateItem[]): void {
  try {
    if (Array.isArray(updates) && updates.length > 0) {
      localStorage.setItem(SYSTEM_UPDATES_LOCAL_KEY, JSON.stringify(updates));
    }
  } catch {}
}

export async function fetchSystemUpdates(): Promise<SystemUpdateItem[] | null> {
  try {
    const res = await fetch('/api/updates');
    if (res.ok) {
      const data = await res.json();
      if (data && Array.isArray(data.updates)) {
        saveLocalUpdates(data.updates);
        return data.updates;
      }
    }
  } catch (e) {
    console.warn('[Clawdock Telemetry] /api/updates fetch warning:', e);
  }
...
export interface LiveConfigFetchResult {
  ...
  timestamp: string;
  isLive: boolean;
  detectedOpenClawVersion?: 'v1' | 'v2';
}

export async function fetchAgentLiveConfig(agentId: AgentId, requestedFormatVersion?: 'v1' | 'v2'): Promise<LiveConfigFetchResult> {
  const fallbackKey = agentId === 'openclaw' && requestedFormatVersion === 'v1' ? 'openclaw-v1' : agentId;
  const fallback = DEFAULT_NATIVE_FILES[fallbackKey] || DEFAULT_NATIVE_FILES['hermes-agent'];
  const fallbackConfig = mergeWithDefaultConfig(agentId);
  const verboseLogs: string[] = [];
  const startTime = Date.now();
  const timestamp = new Date().toLocaleTimeString();

  verboseLogs.push(`[${timestamp}] [INIT] Initiating live container config fetch for agent: "${agentId}"${requestedFormatVersion ? ` (Requested Version: ${requestedFormatVersion})` : ''}`);
  ...
  const vParam = requestedFormatVersion ? `?formatVersion=${requestedFormatVersion}` : '';

  // Primary live endpoint: /api/agents/${agentId}/docker-exec-config
  const endpointsToTry = [
    { url: `/api/agents/${agentId}/docker-exec-config${vParam}`, method: 'GET', desc: 'Docker Exec Live Config (GET)' },
    { url: `/api/agents/${agentId}/docker-exec-config${vParam}`, method: 'POST', desc: 'Docker Exec Live Config (POST)' },
    { url: `/api/agents/${agentId}/config${vParam}`, method: 'GET', desc: 'Agent Config API (GET)' }
  ];
  ...
  if (fetchedData) {
    ...
    if (content) {
      schema = enhanceConfigWithNative(schema, content, format, agentId, requestedFormatVersion);
    }

    const detectedVersion = schema.openclawFormatVersion || (fetchedData.detectedOpenClawVersion as 'v1' | 'v2') || (agentId === 'openclaw' ? 'v2' : undefined);
    ...
    return {
      ...
      timestamp,
      isLive: true,
      detectedOpenClawVersion: detectedVersion
    };
  }
  ...
  try {
    ...
    if (local && local.configs && local.configs[agentId]) {
      ...
      return {
        ...
        timestamp,
        isLive: false,
        detectedOpenClawVersion: agentId === 'openclaw' ? (localSchema.openclawFormatVersion || (requestedFormatVersion || 'v2')) : undefined
      };
    }
  } catch {}
  ...
  return {
    ...
    timestamp,
    isLive: false,
    detectedOpenClawVersion: agentId === 'openclaw' ? (requestedFormatVersion || 'v2') : undefined
  };
}
...
export async function fetchRuntimeAgentStates(): Promise<Record<string, { status: string; containerId: string; logs: string[]; version?: string; dockerImage?: string }>> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return (local as any) || defaultStates;
    }
    const data = await res.json();
    if (data && data.success && data.agentStates) {
      saveLocalAgentStates(data.agentStates);
...
export async function fetchDockerSystemStatus(): Promise<DockerSystemInfo | null> {
  const defaultInfo: DockerSystemInfo = {
  ...
  try {
    ...
    if (res.ok && contentType.includes('application/json')) {
      ...
      if (data && typeof data === 'object') {
        return {
          dockerAvailable: data.dockerAvailable ?? true,
          daemonVersion: data.daemonVersion || data.dockerVersion || defaultInfo.daemonVersion,
          operatingSystem: data.operatingSystem || defaultInfo.operatingSystem,
          totalContainers: typeof data.totalContainers === 'number' ? data.totalContainers : (typeof data.containersTotal === 'number' ? data.containersTotal : defaultInfo.totalContainers),
          runningContainers: typeof data.runningContainers === 'number' ? data.runningContainers : (typeof data.containersRunning === 'number' ? data.containersRunning : defaultInfo.runningContainers),
          socketPath: data.socketPath || defaultInfo.socketPath,
          environment: data.environment || defaultInfo.environment
        };
      }
    } else {
      console.warn(`[Clawdock Docker] Telemetry returned non-JSON response (${res.status}, ${contentType}). Keeping current telemetry state.`);
    }
  } catch (err: any) {
    console.warn('[Clawdock Docker] Docker telemetry endpoint offline or warming up:', err?.message || err);
  }
