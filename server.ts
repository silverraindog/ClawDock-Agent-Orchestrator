  ...
  try {
    ...
    if (provider === 'custom' && (!baseUrl || !baseUrl.trim())) {
      return res.json({
        ...
        isLiveProbed: false,
        models: [
          { value: 'custom-model', label: 'Custom Model (Specify Base URL or model name)', tag: 'Custom' }
        ]
      });
    }

    let liveOllamaModels: string[] = [];
    const isLocalProbe = provider === 'ollama' || Boolean(baseUrl && baseUrl.trim());
    if (isLocalProbe) {
      ...
      if (baseUrl && baseUrl.trim()) {
        ...
        candidateRoots.push('http://ollama:11434');
      }
      ...
      const probeRoot = async (root: string): Promise<string[] | null> => {
        ...
        try {
          ...
          if (resp.ok) {
            ...
            if (Array.isArray(json.models) && json.models.length > 0) {
              const list = json.models.map((m: any) => m.name || m.model).filter(Boolean);
        ...
        try {
          ...
          if (resp.ok) {
            ...
            if (list.length > 0) {
              const listNames = list.map((m: any) => m.id || m.name || m.model).filter(Boolean);
      ...
      try {
        const results = await Promise.allSettled(uniqueRoots.map(r => probeRoot(r)));
        for (const resItem of results) {
          if (resItem.status === 'fulfilled' && resItem.value && resItem.value.length > 0) {
            liveOllamaModels = resItem.value;
            console.log(`[Express API Server] Discovered ${liveOllamaModels.length} models live from endpoint probe:`, liveOllamaModels);
            break;
          }
        }
      } catch {}
...
app.all('/api/proxy/llm', async (req, res) => {
  ...
  try {
    ...
    return res.status(502).json({ error: 'Proxy request failed', message: err?.message || String(err) });
  }
});
...
// OpenClaw VPS Skills & MCP Sync Endpoint
    ...
    mcpServers: [
      {
        id: 'mcp-openclaw-vps-hub',
        name: 'OpenClaw VPS Remote MCP Hub',
        description: 'Connected to https://openclawvps.io/skills/mcp. Provides VPS execution tools and platform webhooks for OpenClaw.',
        transport: 'sse',
        command: 'openclaw-mcp-client',
        args: ['--registry', 'https://openclawvps.io/skills/mcp', '--agent', 'openclaw'],
        env: {
...
// Helper to derive accurate image with tag
function getAgentImageForVersion(agentId: string, version: string): string {
  ...
  const base = baseImages[agentId] || `clawdock/${agentId}`;
  return `${base}:${version}`;
}
...
let dynamicDiscoveredHostContainers: any[] = [];

function queryDockerSocket(): Promise<DockerContainerRaw[]> {
  return new Promise((resolve) => {
    ...
    const req = http.request(
      {
        socketPath,
        path: '/v1.45/containers/json?all=1',
        method: 'GET',
...
function queryDockerCli(): DockerContainerRaw[] {
  try {
    ...
    return lines.map(line => {
      try {
        ...
        return {
          ...
          Status: item.Status || '',
          Created: item.CreatedAt || Date.now(),
          Ports: item.Ports || ''
        };
      } catch {
        return null;
      }
    }).filter(Boolean) as DockerContainerRaw[];
  } catch {
    return [];
  }
}

async function reconcileWithDocker(): Promise<boolean> {
  try {
    let containers = await queryDockerSocket();
    if (containers.length === 0) {
      containers = queryDockerCli();
    }

    if (containers.length > 0) {
      let hasUpdates = false;

      // Update dynamic container list for wildcard search
      dynamicDiscoveredHostContainers = containers.map(c => {
        const cNames = (c.Names || []).map(n => n.replace(/^\//, ''));
        const name = cNames[0] || (c.Id || '').substring(0, 12);
        const image = c.Image || 'host-image';
        const cId = (c.Id || '').substring(0, 12);
        const isUp = (c.State === 'running') || (typeof c.Status === 'string' && c.Status.toLowerCase().startsWith('up'));
        let suggestedAgentId: string | undefined = undefined;
        if (name.includes('openclaw') || image.includes('openclaw')) suggestedAgentId = 'openclaw';
        else if (name.includes('picoclaw') || image.includes('picoclaw')) suggestedAgentId = 'picoclaw';
        else if (name.includes('zeroclaw') || image.includes('zeroclaw')) suggestedAgentId = 'zeroclaw';
        else if (name.includes('hermes') || image.includes('hermes') || image.includes('nous')) suggestedAgentId = 'hermes-agent';

        let portsStr = '';
        if (Array.isArray(c.Ports)) {
          portsStr = c.Ports.map(p => `${p.PublicPort ? `${p.IP || '0.0.0.0'}:${p.PublicPort}->` : ''}${p.PrivatePort}/${p.Type}`).join(', ');
        ...
        return {
          ...
          state: c.Status || (isUp ? 'Up' : 'Exited'),
          created: typeof c.Created === 'number' ? new Date(c.Created * 1000).toLocaleString() : String(c.Created || 'recently'),
          ports: portsStr,
          suggestedAgentId,
          matchReason: suggestedAgentId ? `Detected live host container matching ${suggestedAgentId}` : 'Host container',
          confidence: suggestedAgentId ? 'high' : 'medium'
        };
      });

      for (const c of containers) {
        const cNames = (c.Names || []).map(n => (n || '').toLowerCase());
    ...
    console.warn('[Docker Engine Reconciler] Query error:', err);
  }
  return false;
}

const DEFAULT_SYSTEM_UPDATES = [
  {
  ...
  },
  {
    ...
    packageOrImage: 'mcp/docker-engine'
  },
  {
    id: 'update-mcp-puppeteer',
    name: 'Puppeteer Headless MCP',
    category: 'mcp',
    targetId: 'mcp-puppeteer',
  ...
  },
  {
    ...
    status: 'update_available',
    lastChecked: 'Just now',
    packageOrImage: 'skills/web-search@1.1.0'
  },
  {
    ...
    status: 'update_available',
    lastChecked: 'Just now',
    packageOrImage: 'skills/python-exec@1.3.0'
  },
  {
    id: 'update-skill-git-master',
    name: 'Git Automation Hub',
    category: 'skill',
    ...
    status: 'update_available',
    lastChecked: '10 mins ago',
    packageOrImage: 'skills/git-master@1.1.2'
  },
  {
    id: 'update-skill-memory-graph',
    name: 'EverOS Memory Graph Sync',
    category: 'skill',
    ...
    status: 'update_available',
    lastChecked: '15 mins ago',
    packageOrImage: 'skills/memory-graph@1.0.0'
  },
  {
    id: 'update-skill-shell-exec',
    name: 'Container Terminal & Shell',
    category: 'skill',
    ...
    packageOrImage: 'skills/shell-exec@1.2.0'
  },
  {
    ...
    status: 'update_available',
    lastChecked: '1 hour ago',
    packageOrImage: 'skills/sql-query@1.0.5'
  },
  {
    ...
    status: 'update_available',
    lastChecked: '2 hours ago',
    packageOrImage: 'skills/media-vision@0.9.0'
  },
  {
    ...
    status: 'update_available',
    lastChecked: '3 hours ago',
    packageOrImage: 'skills/iot-mqtt@0.6.0'
  }
];

let systemUpdatesStore: any[] = JSON.parse(JSON.stringify(DEFAULT_SYSTEM_UPDATES));
...
function loadPersistentState() {
  try {
    if (fs.existsSync(STATE_FILE_PATH)) {
      ...
      if (data && Array.isArray(data.systemUpdatesStore)) {
        const savedMap = new Map<string, any>(data.systemUpdatesStore.map((u: any) => [u.id, u]));
        systemUpdatesStore = systemUpdatesStore.map(item => {
          const saved = savedMap.get(item.id);
          if (saved) {
            return {
              ...item,
              currentVersion: saved.currentVersion || item.currentVersion,
              latestVersion: saved.latestVersion || item.latestVersion,
              status: saved.status || item.status,
              lastChecked: saved.lastChecked || item.lastChecked
            };
          }
          return item;
        });

        for (const s of data.systemUpdatesStore) {
          if (!systemUpdatesStore.some(u => u.id === s.id)) {
            systemUpdatesStore.push(s);
          }
        }
      }
    }
    // Ensure all registered agents with containerId preserve running status across sessions
    for (const [id, st] of Object.entries(agentStates)) {
      // Apply updated version and docker image to agent state if matching update is found
      const matchingUpdate = systemUpdatesStore.find(u => u.targetId === id && u.status === 'up_to_date');
      if (matchingUpdate) {
        (st as any).version = matchingUpdate.currentVersion;
        (st as any).dockerImage = getAgentImageForVersion(id, matchingUpdate.currentVersion);
        // An updated container was deployed with the new image, so ensure it is running
        st.status = 'running';
      } else {
        const currentVer = (st as any).version || 'v1.0.0';
        if (!(st as any).dockerImage) {
          (st as any).dockerImage = getAgentImageForVersion(id, currentVer);
        }
      }

      if ((id === 'picoclaw' || id === 'openclaw' || id === 'zeroclaw' || id === 'hermes-agent') && st.containerId) {
        if (id === 'openclaw' || id === 'picoclaw' || id === 'hermes-agent') {
          st.status = 'running';
        }
      }
    }
  ...
  'openclaw': {
    ...
    content: JSON.stringify({
      ...
      "agents": {
        ...
        "entries": {
          "openclaw": {
            ...
            "description": "Multi-channel cooperative assistant gateway and coordinator",
            "workspace": "/workspace",
            "model": "openai/gpt-4o",
            "runtime": {
              "type": "embedded"
      ...
      "channels": {
        ...
        "discord": {
          "enabled": true,
          "token": "env:OPENCLAW_DISCORD_TOKEN",
...
app.all('/api/state', async (req, res, next) => {
  ...
  if (req.method === 'GET') {
    await reconcileWithDocker().catch(() => {});
    return res.json({
      success: true,
      agentStates,
      timestamp: new Date().toISOString()
...
app.all(['/api/docker/status', '/api/docker/status/'], async (req, res) => {
  if (req.method === 'OPTIONS') {
    return res.sendStatus(200);
  }
  await reconcileWithDocker().catch(() => {});
  ...
  return res.json({
    ...
    totalContainers: Math.max(6, runningCount + 2),
...
function getAgentConfigData(agentId: string, requestedFormatVersion?: string) {
  ensureNativeConfigFiles();

  const isExplicitV1 = requestedFormatVersion === 'v1' || (agentId === 'openclaw' && requestedFormatVersion === 'v1');
  const agentStateVer = agentStates[agentId]?.version || '';
  const isAgentV1 = agentId === 'openclaw' && agentStateVer.startsWith('v1.');

  let def = DEFAULT_NATIVE_FILES[agentId] || DEFAULT_NATIVE_FILES['hermes-agent'];
  if (agentId === 'openclaw' && (isExplicitV1 || (isAgentV1 && requestedFormatVersion !== 'v2'))) {
    def = DEFAULT_NATIVE_FILES['openclaw-v1'] || def;
  }
    ...
    if (nativeFormat === 'json') {
      ...
      if (agentId === 'openclaw') {
        const isV2 = Boolean(
          parsedJson.meta ||
          parsedJson.gateway ||
          parsedJson.agents?.defaults ||
          parsedJson.agents?.entries ||
          parsedJson.models?.providers ||
          (parsedJson.version && String(parsedJson.version).startsWith('2.'))
        );
        detectedOpenClawVersion = isV2 ? 'v2' : 'v1';

        if (isV2) {
          const entry = parsedJson.agents?.entries?.openclaw || parsedJson.agents?.entries?.[Object.keys(parsedJson.agents?.entries || {})[0]];
          if (entry?.name) parsedAgentName = entry.name;
          if (entry?.description) parsedSystemPrompt = entry.description;

          const modelRef = entry?.model || parsedJson.agents?.defaults?.model;
          const modelStr = typeof modelRef === 'string' ? modelRef : modelRef?.primary || '';
          if (modelStr.includes('/')) {
            const parts = modelStr.split('/');
            parsedModelProvider = parts[0];
            parsedModelName = parts.slice(1).join('/');
          } else if (modelStr) {
            parsedModelName = modelStr;
          }
        } else {
  ...
  return {
    ...
    nativeFormat,
    nativeContent,
    detectedOpenClawVersion: agentId === 'openclaw' ? detectedOpenClawVersion : undefined,
    ...
    configSchema: {
      agentId,
      version: '1.0.0',
      openclawFormatVersion: agentId === 'openclaw' ? detectedOpenClawVersion : undefined,
      model: {
...
app.get('/api/agents/:id/config', (req, res) => {
  try {
    ...
    const formatVersion = (req.query.formatVersion || req.query.version) as string;
    if (agentId === 'all') {
      const ids = ['hermes-agent', 'zeroclaw', 'openclaw', 'picoclaw'];
      const allConfigs: Record<string, any> = {};
      for (const id of ids) {
        try {
          allConfigs[id] = getAgentConfigData(id, formatVersion);
        } catch {}
      }
      return res.json({ success: true, configs: allConfigs });
    }
    const result = getAgentConfigData(agentId, formatVersion);
    res.json(result);
  } catch (e: any) {
...
app.get('/api/docker/containers/search', async (req, res) => {
  await reconcileWithDocker().catch(() => {});
  const query = (req.query.pattern as string || '').toLowerCase().trim();
  const wildcardKeywords = query ? query.split(/[,* ]+/).filter(Boolean) : [];

  // Merge dynamic containers with static fallback, deduplicating by ID
  const allContainers = [...dynamicDiscoveredHostContainers];
  for (const staticC of discoveredHostContainers) {
    if (!allContainers.some(c => c.id === staticC.id || c.name === staticC.name)) {
      allContainers.push(staticC);
    }
  }

  let results = allContainers.map(c => {
...
app.post('/api/agents/:id/install', (req, res) => {
  ...
  const currentVersion = (agentStates[agentId] as any)?.version || 'v1.0.0';
  const currentImage = (agentStates[agentId] as any)?.dockerImage || getAgentImageForVersion(agentId, currentVersion);
  ...
  agentStates[agentId] = {
    ...
    logs: [
      `[Docker Engine] Pulling image ${currentImage}...`,
  ...
  res.json({
    success: true,
    message: `Installed and started ${agentId} inside Docker container ${newContainerId} (${currentImage})`,
    containerId: newContainerId,
    dockerImage: currentImage,
    version: currentVersion,
    status: 'running'
...
// Run doctor fix for agent (e.g. OpenClaw state database schema migration and gateway restart)
app.all(['/api/agents/:id/doctor-fix', '/api/agents/:id/doctor-fix/'], (req, res) => {
  const agentId = req.params.id;
  const timestamp = new Date().toLocaleTimeString();
  console.log(`[Express API Server] [${timestamp}] ${req.method} /api/agents/${agentId}/doctor-fix invoked`);

  if (!agentStates[agentId]) {
    agentStates[agentId] = { status: 'stopped', containerId: 'c_' + Math.random().toString(36).substring(2, 8), logs: [] };
  }

  const oldStatus = agentStates[agentId].status;
  agentStates[agentId].status = 'running';

  agentStates[agentId].logs.push(
    `[${timestamp}] [orchestrator] Initiating doctor fix routine for ${agentId}...`,
    `[${timestamp}] [docker] Executing one-off container: docker run --rm -v /home/${agentId}:/home/${agentId} ghcr.io/openclaw/openclaw:latest ${agentId} doctor --fix`
  );

  // If host docker socket is active, execute one-off fix container and start/restart agent
  if (fs.existsSync('/var/run/docker.sock')) {
    try {
      const fixCmd = `docker run --rm -v /home/${agentId}:/home/${agentId} ghcr.io/openclaw/openclaw:latest openclaw doctor --fix`;
      execSync(fixCmd, { timeout: 15000, stdio: ['ignore', 'pipe', 'pipe'] });
      agentStates[agentId].logs.push(`[${timestamp}] [openclaw doctor] Database schema migrated successfully (audit-events-v2).`);
    } catch (err: any) {
      console.log(`[Doctor Fix] One-off container execution note: ${err.message}`);
    }

    if (agentStates[agentId].containerId) {
      try {
        execSync(`docker start ${agentStates[agentId].containerId} || docker restart ${agentStates[agentId].containerId}`, { timeout: 5000, stdio: ['ignore', 'pipe', 'ignore'] });
      } catch {}
    }
  }

  agentStates[agentId].logs.push(
    `[${timestamp}] [state/db] State database schema migrated successfully (audit-events-v2) at /home/${agentId}/state/${agentId}.sqlite`,
    `[${timestamp}] [gateway] Gateway daemon restarted successfully. Status: RUNNING on port 8082.`
  );
...
app.all(['/api/export/code', '/api/export/code/'], (req, res) => {
  const files = [
    {
  ...
  const isZip = accept.includes('application/zip') || req.query.format === 'zip';
  ...
  return res.json({
    ...
    downloadUrl: '/api/export/code?format=zip',
...
app.post('/api/updates/apply', (req, res) => {
  ...
  let found = systemUpdatesStore.find(u => u.id === id);
  if (!found) {
    found = {
      id,
      name: id,
      targetId: id.replace('update-agent-', '').replace('update-mcp-', 'mcp-').replace('update-skill-', 'skill_'),
      currentVersion: targetVersion || 'v1.0.0',
      latestVersion: targetVersion || 'v1.0.0',
      status: 'up_to_date',
      lastChecked: 'Just now',
      packageOrImage: ''
    };
    systemUpdatesStore.push(found);
  } else {
    found.currentVersion = targetVersion || found.latestVersion;
    found.status = 'up_to_date';
    found.lastChecked = 'Just now';
  }
  
  const targetAgentId = found.targetId || id.replace('update-agent-', '');
  if (agentStates[targetAgentId]) {
    const newVersion = found.currentVersion;
    const newImage = getAgentImageForVersion(targetAgentId, newVersion);
    const newContainerId = 'dck_' + Math.random().toString(36).substring(2, 10);
    const oldContainerId = agentStates[targetAgentId].containerId || 'c_prev';

    (agentStates[targetAgentId] as any).version = newVersion;
    (agentStates[targetAgentId] as any).dockerImage = newImage;
    agentStates[targetAgentId].containerId = newContainerId;
    agentStates[targetAgentId].status = 'restarting';
    
    agentStates[targetAgentId].logs.push(
      `[${new Date().toLocaleTimeString()}] [Update Engine] Pulling updated container image ${newImage}...`,
      `[${new Date().toLocaleTimeString()}] [Docker Engine] Layer verified sha256:${Math.random().toString(16).substring(2, 10)}. Image ready.`,
      `[${new Date().toLocaleTimeString()}] [Docker Engine] Stopping and removing deprecated container ${oldContainerId}...`,
      `[${new Date().toLocaleTimeString()}] [Docker Engine] Created container ${newContainerId} using image ${newImage}.`,
      `[${new Date().toLocaleTimeString()}] [Docker Engine] Spawning runtime with volume bindings /workspace and persistent memory.`
    );
    
    if (oldContainerId) {
      try {
        execSync(`docker restart ${oldContainerId}`, { timeout: 4000, stdio: ['ignore', 'pipe', 'ignore'] });
      } catch {}
    }
    ...
    setTimeout(() => {
      if (agentStates[targetAgentId]) {
        ...
        agentStates[targetAgentId].logs.push(
          `[${new Date().toLocaleTimeString()}] [Docker Engine] Container ${newContainerId} (${targetAgentId} ${newVersion}) healthy and listening.`
        );
        savePersistentState();
      }
    }, 800);
  ...
  res.json({
    ...
    item: found,
    message: `Successfully applied update and recreated container runtime.`
  });
});

// Apply all pending updates
app.post('/api/updates/apply-all', (req, res) => {
  const { items } = req.body || {};
  const idsToUpdate = Array.isArray(items) && items.length > 0 
    ? items 
    : systemUpdatesStore.filter(u => u.status === 'update_available').map(u => u.id);
  
  const updatedItems: any[] = [];
  for (const u of systemUpdatesStore) {
    if (idsToUpdate.includes(u.id)) {
      u.currentVersion = u.latestVersion;
      u.status = 'up_to_date';
      u.lastChecked = 'Just now';
      updatedItems.push(u);

      const targetAgentId = u.targetId;
      if (targetAgentId && agentStates[targetAgentId]) {
        const newVersion = u.currentVersion;
        const newImage = getAgentImageForVersion(targetAgentId, newVersion);
        const newContainerId = 'dck_' + Math.random().toString(36).substring(2, 10);
        const oldContainerId = agentStates[targetAgentId].containerId || 'c_prev';
        ...
        agentStates[targetAgentId].logs.push(
          `[${new Date().toLocaleTimeString()}] [Update Engine] Bulk update pulled image ${newImage}`,
          `[${new Date().toLocaleTimeString()}] [Docker Engine] Container ${oldContainerId} replaced by ${newContainerId} running ${newImage}`,
          `[${new Date().toLocaleTimeString()}] [Docker Engine] Container runtime healthy and operational.`
        );
  ...
  res.json({
    ...
    updates: systemUpdatesStore,
    message: `Applied ${updatedItems.length} updates successfully and recreated container instances.`
  });
});
